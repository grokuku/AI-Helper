...
Couvre :
  ...
  (+) unitaires _call_llm_internal / _normalize_tool_calls.
...
from routes.enhance import _call_llm_internal, _normalize_tool_calls
...
class TestToolCallsRelay:
    def test_pure_tool_call_relayed(self, client, auth_headers):
        """content null + tool_calls => 200 avec champ tool_calls normalisé."""
        _ensure_user()
        ...
        body = r.get_json()
        assert body["tool_calls"] == [NORMALIZED_TOOL_CALL]
...
class TestCallLlmInternal:
    ...
    def test_tool_calls_response_normalized_and_not_retried(self):
        ...
        with mock.patch("requests.post", return_value=_FakeResp(json_data=provider)) as mpost:
            result = _call_llm_internal(self.LLM_REQ, self.LLM_CONF)
        assert result["tool_calls"] == [NORMALIZED_TOOL_CALL]
...
class TestNormalizeToolCalls:
    def test_none_for_text_message(self):
        assert _normalize_tool_calls({"content": "salut"}) is None
        assert _normalize_tool_calls({"content": "salut", "tool_calls": []}) is None
        assert _normalize_tool_calls({"content": None}) is None

    def test_normalization_fields(self):
        out = _normalize_tool_calls({"tool_calls": [PROVIDER_TOOL_CALL]})
        assert out == [NORMALIZED_TOOL_CALL]

    def test_missing_id_and_arguments_default(self):
        out = _normalize_tool_calls(
            {"tool_calls": [{"type": "function", "function": {"name": "f"}}]}
        )
        assert out == [{"id": "", "name": "f", "arguments": ""}]

    def test_non_string_arguments_coerced_to_empty(self):
        out = _normalize_tool_calls(
            {"tool_calls": [{"id": "c1", "function": {"name": "f", "arguments": 42}}]}
        )
        assert out == [{"id": "c1", "name": "f", "arguments": ""}]

    def test_non_dict_entries_skipped(self):
        out = _normalize_tool_calls({"tool_calls": ["junk", PROVIDER_TOOL_CALL]})
        assert out == [NORMALIZED_TOOL_CALL]

    def test_non_dict_message_safe(self):
        assert _normalize_tool_calls("not-a-dict") is None
