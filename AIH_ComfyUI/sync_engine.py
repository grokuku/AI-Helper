...
Auth : Bearer (api_key lue depuis credentials.json via
AIH_ComfyUI/nodes/_credentials.py). Aucune dépendance externe (urllib).
"""
...
# Import robuste de store.py :
# - Dans le runtime ComfyUI, le package AIH_ComfyUI est pré-enregistré dans
#   sys.modules par _load_module() → `from AIH_ComfyUI import store` marche.
# - Hors runtime (tests, import direct), on retombe sur un chargement par
#   chemin absolu pour ne pas exécuter AIH_ComfyUI/__init__.py.
try:
    from AIH_ComfyUI import store
except Exception:  # pragma: no cover - fallback hors contexte ComfyUI
    _store_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "store.py")
    _spec = importlib.util.spec_from_file_location("aih_store", _store_path)
    if _spec is None or _spec.loader is None:
        raise ImportError("store.py introuvable dans AIH_ComfyUI/")
    _store_mod = importlib.util.module_from_spec(_spec)
    sys.modules["aih_store"] = _store_mod
...
def _load_credentials():
    """Charge api_url + api_key via _credentials.py.

    Retourne:
        tuple: (api_url, api_key) ou (None, None) si indisponible.
    """
    try:
        from AIH_ComfyUI.nodes import _credentials as creds
        return creds.get_api_url(), creds.get_api_key()
    except Exception:
        pass
    # Fallback hors runtime ComfyUI : chargement direct par chemin.
    try:
        cred_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "nodes", "_credentials.py"
        )
        spec = importlib.util.spec_from_file_location("aih_credentials", cred_path)
