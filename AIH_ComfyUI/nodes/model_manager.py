"""
AIH Model Manager — List, upload et download de models/loras.
...
Endpoints ajoutés au serveur ComfyUI (aiohttp) :
  GET  /aih/models/list           → liste des models/loras locaux + tailles
  POST /aih/models/upload         → upload un model vers le serveur AIH (chunked)
  POST /aih/models/download       → download un model depuis AIH → sauvegarde locale

L'upload se fait en streaming : le Python lit le fichier par chunks et les envoie
au backend AIH via /api/files/init + /api/files/chunk + /api/files/complete.
...
def _get_config():
    """Lit la configuration AIH (URL du serveur + API key).
...
def _get_aih_credentials():
    """Lit les credentials AIH pour connaître l'URL du serveur + API key."""
    try:
        ...
        try:
            ...
            cred_file = os.path.join(user_dir, "default", "aih_credentials.json")
