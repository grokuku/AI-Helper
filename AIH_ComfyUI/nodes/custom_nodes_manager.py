"""
AIH Custom Nodes Manager — Détection et installation de custom nodes.

Endpoints ajoutés au serveur ComfyUI (aiohttp) :
  GET  /aih/custom-nodes          → liste des nodes installés + URLs git
  POST /aih/custom-nodes/install  → git clone d'un node manquant

Usage côté JS (aih_workflow_share.js) :
  - Au partage : fetch('/aih/custom-nodes') pour enrichir required_nodes avec les URLs
  - À l'install : fetch('/aih/custom-nodes/install', {url, name}) pour cloner
"""
        ...
        if os.path.isfile(req_file):
            try:
                ...
                if pip_result.returncode != 0:
                    logging.warning(f"[AIH] pip install failed for {name}: {pip_result.stderr[:200]}")
            except Exception as e:
                logging.warning(f"[AIH] pip install error for {name}: {e}")
